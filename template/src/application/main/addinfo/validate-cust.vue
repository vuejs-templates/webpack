      ...
      <template  slot="val">
        <div class="right">
          <za-date @onChange="onBeginDateChange" class="begindate" :formModel="begindateModel" :inset="true" :index="index" name="begindate"></za-date>
           <check-list labelPosition="right" required :options="prolist" v-model="checkList" :inset="true" :index="index" name="checklong" ></check-list>
        </div>
      </template>
    </row-2-col>
  </div>
...
<script>
  import {zaInput, zaSelect, row2Col, baseMixin, zaDate, zaYesno, checkList} from 'vformer'
  ...
  export default {
    ...
    computed: {
      ...
      checkList() {
        return {
          value: this.formModel.value.checklong,
          rules: this.formModel.rules.checklong
        }
      }
    },
    methods: {
      ...
      onValidate() {
        return new Promise((resolve, reject) => {
          let isValid = (!!this.innerValue.begindate && !!this.innerValue.checklong)
